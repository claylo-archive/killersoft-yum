...
%{__install} -m 0755 src/Linux_All_OPT.OBJ/libjs.so \
    %{buildroot}%{_libdir}/js/
%{__mv} %{buildroot}%{_libdir}/js/libjs.so %{buildroot}%{_libdir}/js/libjs.so.1
%{__ln_s} -nf libjs.so.1 %{buildroot}%{_libdir}/js/libjs.so
...
%changelog
* Mon Jul  6 2009 Clay Loveless <clay@killersoft.com> - 1.70-4.ks
- Altered install path to work with PHP spidermonkey extension
